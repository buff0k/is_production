...
def execute(filters=None):
    ...
    """
def build_site_graph(site, start_dt, end_dt):
    ...
    for exc in excavators:
        bcm, hours = get_excavator_bcm_and_hours(site, exc, start_dt, end_dt)
...
def get_excavator_bcm_and_hours(site, excavator, start_dt, end_dt):
    rows = frappe.db.sql("""
        SELECT
            hp.name,
        ...
        WHERE hp.location = %s
          AND hp.prod_datetime BETWEEN %s AND %s
          AND tl.asset_name_shoval = %s
        GROUP BY hp.name
    """, (site, start_dt, end_dt, excavator), as_dict=True)

    total_bcm = sum(r.bcm for r in rows)
    hours = len(rows)  # each HP record = one hour

...
@frappe.whitelist()
def get_batch_elevation_points(
	geo_project=None,
	version_tag=None,
	variable_name=None,
	import_batch=None,
	...
	"""Return imported batch X/Y/Z values for the contour viewer.

	This is a convenience wrapper around get_geo_points() using Geo Model data.
	The frontend may still call get_geo_points() directly so the page stays compatible
	with the existing mining block workflow.
	"""
	return get_geo_points(
		data_source="Geo Model",
		geo_project=geo_project,
		version_tag=version_tag,
		variable_name=variable_name,

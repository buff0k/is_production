...
class GeoCalculationBatch(Document):
    ...
    def set_defaults(self):
        ...
        if self.replace_existing in (None, ""):
    ...
    def validate_required_fields(self):
        if not self.batch_id:
            frappe.throw("Batch ID is required.")
        ...
        if not self.calculation_name:
            frappe.throw("Calculation Name is required.")

        if not self.source_mode:
            frappe.throw("Source Mode is required.")

        if not self.calculation_type:
            frappe.throw("Calculation Type is required.")

        if self.source_mode == "Attached Files" and not self.geo_model_output:
            frappe.throw("Geo Model Output is required when using Attached Files.")
...
def _get_import_batch_file(import_batch_name, side):
    ...
    for fieldname in (
        ...
        if hasattr(import_batch, fieldname) and import_batch.get(fieldname):
            file_url = import_batch.get(fieldname)
            break

    if not file_url:
        frappe.throw(f"{side.title()} Import Batch {import_batch_name} has no attached source file.")
...
def _get_source_file_path(batch, side):
    ...
    if source_mode == "Existing Import Batches":
        ...
        return _get_import_batch_file(import_batch_name, side)
...
def enqueue_create_calculated_points(docname, replace_existing=1):
    ...
    frappe.db.set_value(
        ...
        docname,
        {
            ...
            "calculation_log": f"Background calculated points job queued at {now()}.\nJob ID: {job_id}",
        },
...
def _validate_calculation_batch(batch):
    ...
    if batch.source_mode == "Attached Files":
        if not batch.geo_model_output:
            frappe.throw("Geo Model Output is required when using Attached Files.")

        if not batch.reference_file_attachment:
            frappe.throw("Reference File Attachment is required.")

        if not batch.target_file_attachment:
            frappe.throw("Target File Attachment is required.")

    if batch.source_mode == "Existing Import Batches":
        if not batch.reference_import_batch:
            frappe.throw("Reference Import Batch is required.")

        if not batch.target_import_batch:
            frappe.throw("Target Import Batch is required.")


@frappe.whitelist()
def create_calculated_points(docname, replace_existing=1):
    batch = frappe.get_doc("Geo Calculation Batch", docname)
    batch.set_full_names()
    _validate_calculation_batch(batch)
    ...
    if int(replace_existing or 0):
        deleted_existing_count = _delete_existing_points(batch)
    ...
    for index, target_point in enumerate(target_points, start=1):
        try:
            ...
            _set_if_field(
                ...
                "remarks",
                (
                    f"{batch.calculated_variable_code} calculated using {calculation_type}; "
                    f"reference {batch.reference_variable_code}={reference_z}; "
                    f"target {batch.target_variable_code}={target_z}; "
                    f"calculated {calculated_z}; key {key}"
                ),
            )
        ...
        docname,
        {
            ...
            "calculation_log": "\n".join(
                [
                    ...
                    f"Deleted Existing Points: {deleted_existing_count}",
                    "",
                    f"Reference rows: {reference_row_count}",
                    f"Target rows: {target_row_count}",
